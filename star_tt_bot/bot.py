...
try:
    from . import star_client as _sc
except ImportError:
...
class StarTeamTalkBot:
    ...
    def connect(self):
        ...
        log.info("Connection initiated; waiting for CLIENTEVENT_CON_SUCCESS...")
        if not self._wait_connection(timeout=15):
            err = self._safe_last_error()
            raise RuntimeError(f"Never received connection-success from {self.host}:{self.tcp_port}"
                               + (f" (server error {err})" if err else ""))
        log.info("Connected. Logging in as nickname=%r username=%r",
    ...
    def _wait_connection(self, timeout=15):
        """Pump events until CLIENTEVENT_CON_SUCCESS (True) or a failure
        (CON_FAILED / CON_LOST) event (False). Per the TeamTalk SDK client
        programming guide, Connect() is asynchronous -- it returns True only
        meaning 'connection initiated', and you must wait for the
        CON_SUCCESS event before issuing DoLogin()."""
        import time
        ...
        while time.time() < deadline:
            ...
            if not m:
                continue
            ev = m.nClientEvent
            if ev == int(sdk.ClientEvent.CLIENTEVENT_CON_SUCCESS):
                return True
            if ev == int(sdk.ClientEvent.CLIENTEVENT_CON_FAILED):
                log.error("Connection to server FAILED (CLIENTEVENT_CON_FAILED).")
                return False
            if ev == CLIENTEVENT_CON_LOST:
                log.error("Connection lost while waiting for connect success.")
                return False
        return False
    ...
    def _login_and_sync(self, timeout=20):
        ...
        while time.time() < deadline:
            do_login_calls += 1
            rc = self.tt.doLogin(self.nickname, self.username, self.password, self.client_name)
            if rc < 0:
                # doLogin() returns < 0 ONLY if the command could not be ISSUED
                # (e.g. not connected). It does NOT mean credentials were
                # rejected -- that arrives later as a CLIENTEVENT_CMD_ERROR.
                log.warning("doLogin() could not be issued (returned %s, attempt %d); retrying...",
                            rc, do_login_calls)
                _note(-3, f"doLogin rc={rc}")
                time.sleep(2)
                continue
            log.info("doLogin() command issued (attempt %d); awaiting authorization...", do_login_calls)
            _note(0, "doLogin issued")
            ...
            while time.time() < end:
                if self.tt.getMyUserID() != 0:
                    ...
                    if self.status and not status_set:
                        try:
                ...
                if ev == CLIENTEVENT_CMD_MYSELF_LOGGEDIN:
                    ...
                    if code != 0:
                        ...
                        if code == 3001:  # CMDERR_ALREADY_LOGGEDIN
                            ...
                            do_login_rejected += 1
                            time.sleep(5)
                            break  # break inner pump; outer loop re-issues doLogin
            ...
            # Inner pump loop ended (timeout or break)
            if logged_in:
                log.info("Login confirmed; waiting for channel tree...")
                grace_end = time.time() + 3.0
                while time.time() < grace_end:
                    m2 = self.tt.getMessage(150)
                    if not m2:
                        continue
                    if m2.nClientEvent == int(sdk.ClientEvent.CLIENTEVENT_CMD_CHANNEL_NEW) and m2.channel:
                        ch = m2.channel
                        self._channel_tree[ch.nChannelID] = (sdk.ttstr(ch.szName), ch.nParentID)
                    elif m2.nClientEvent == int(sdk.ClientEvent.CLIENTEVENT_CMD_CHANNEL_UPDATE) and m2.channel:
                        ch = m2.channel
                        self._channel_tree[ch.nChannelID] = (sdk.ttstr(ch.szName), ch.nParentID)
                log.info("Channel tree collected: %d channels, root_id=%d",
                         len(self._channel_tree), self.tt.getRootChannelID())
                return
            _note(-2, "doLogin issued but not authorized in 15s")
            # login command accepted but no confirmation; outer loop will retry
        ...
        diag = (
            ...
            f"  authorized flag: {self._safe_authorized()}\n"
    ...
    def _safe_authorized(self):
        try:
            return (self.tt.getFlags() & int(sdk.ClientFlags.CLIENT_AUTHORIZED)) != 0
        except Exception as e:
            return f"<error: {e}>"
    ...
    def _join_channel(self):
        log.info("Attempting to join channel: '%s'", self.channel_path)
        log.info("Root channel ID: %d", self.tt.getRootChannelID())
        
        # Debug: list all known channels
        try:
            chans = self.tt.getServerChannels()
            log.info("Known channels (%d):", len(chans))
            for ch in chans:
                name = sdk.ttstr(ch.szName)
                log.info("  id=%d parent=%d name=%r", ch.nChannelID, ch.nParentID, name)
        except Exception as e:
            log.warning("Could not list channels: %s", e)

        chan_id = self.tt.getChannelIDFromPath(self.channel_path)
        log.info("getChannelIDFromPath('%s') -> %d", self.channel_path, chan_id)
        
        if chan_id <= 0:
            chan_id = self.tt.getRootChannelID()
            log.warning("Channel not found, falling back to root channel id=%d", chan_id)
        if chan_id <= 0:
            log.error("Channel '%s' not found and no root channel visible.", self.channel_path)
            return
        ...
        for i in range(60):
            my_id = self.tt.getMyChannelID()
            if my_id == chan_id:
                log.info("Successfully joined channel %d", chan_id)
                return
            if i % 10 == 0:
                log.debug("Waiting for join... my_channel_id=%d target=%d", my_id, chan_id)
            self.tt.getMessage(100)
        log.warning("Join not confirmed after 6s; my_channel_id=%d", self.tt.getMyChannelID())
